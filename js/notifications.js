...
let notifications = [
  {
    ...
    message:
      "Website AFANLY saat ini masih dalam tahap pengembangan. Beberapa fitur mungkin belum berfungsi sempurna.",
    time: "Baru saja",
    timestamp: Date.now(),
    read: false,
    actions: [
  ...
  },
  {
    ...
    message:
      "Terima kasih telah mengunjungi website AFANLY. Jelajahi layanan dan produk IT kami.",
  ...
  saveNotifications() {
    try {
      localStorage.setItem(
        "afanly-notifications",
        JSON.stringify(notifications.map((n) => ({ id: n.id, read: n.read })))
      );
    } catch (e) {
                            ...
                            <div class="notification-actions">
                                ${notif.actions
                                  .map(
                                    (action) => `
                                    <button class="btn-${
                                      action.callback === "dismissNotification"
                                        ? "dismiss"
                                        : "primary-small"
                                    }" 
  ...
  handleAction(action, notifId) {
    switch (action) {
      ...
      case "contactDirect":
        ...
        this.closePanel();
        window.open(
          `https://wa.me/${CONFIG?.whatsappNumber || "628127659073"}`,
          "_blank"
        );
        break;
      ...
      case "copyEmail":
        this.markAsRead(notifId);
        const email = CONFIG?.emailAddress || "defry.pku@gmail.com";
      ...
      case "viewOrder":
        this.markAsRead(notifId);
        this.closePanel();
        // Redirect to orders page or section
        break;
    }
  },
